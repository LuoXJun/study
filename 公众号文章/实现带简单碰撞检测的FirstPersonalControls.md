### 实现带简单碰撞检测的FirstPersonalControls
1.实现鼠标控制相机镜头转动
```javaScript
// 设置欧拉角以及旋转顺序
...
```
+ 不含惯性和碰撞检测
![avatar](https://pic.imgdb.cn/item/61e683fc2ab3f51d911278a2.gif)
```javaScript
    ...
    moveForwardFun( - velocity.z )
```
+ 带惯性
![avatar](https://pic.imgdb.cn/item/61e6834e2ab3f51d91119d74.gif)
```javaScript
...
```
+ 带碰撞检测和惯性
![avatar](https://pic.imgdb.cn/item/61e6848e2ab3f51d911322c6.gif)
```javaScript
if( scope.enableDamping){
...
// 碰撞检测
//同时按下两个按键时，碰撞检测方向不再是前后左右，变为了左前右前左后右后
if(direction.z > 0){
    if(direction.x > 0){
...
//CollisionDistance:相机和碰撞检测物体之间的距离 
